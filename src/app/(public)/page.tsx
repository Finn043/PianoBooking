    ...
    <main id="main-content" className="overflow-hidden bg-white text-[#172523]">
      ...
      <header className="absolute inset-x-0 top-0 z-20">
        <nav aria-label="Main navigation" className="mx-auto flex max-w-[90rem] items-center justify-between px-5 py-6 md:px-10 lg:px-16">
          ...
          <div className="hidden items-center gap-8 text-base font-medium text-white/90 md:flex"><a href="#approach" className="hover:text-white">Lessons</a><a href="#booking" className="hover:text-white">Book a lesson</a><a href="/admin/login" className="border-b border-white/50 pb-1 hover:border-white">Teacher login</a></div>
        ...
        {menuOpen && <div className="mx-5 flex flex-col bg-[#173c38] p-6 text-white md:hidden">{[['Lessons', '#approach'], ['Book a lesson', '#booking'], ['Teacher login', '/admin/login']].map(([label, href]) => <a key={label} href={href} onClick={() => setMenuOpen(false)} className="border-b border-white/15 py-4 text-lg">{label}</a>)}</div>}
      </header>
      ...
      {selectedSlot && <BookingForm slot={selectedSlot} userTimezone="Australia/Melbourne" onClose={() => { setSelectedSlot(null); fetchAvailableSlots(); }} />}
    </main>
  );
}
